...
export default function DeleteWatchlistModal({ id, name }: WatchlistItemProps) {
  const INTIAL_FORMDATA = useMemo(() => ({ name: "" }), []);
        ...
        <Box maw={340} mx="auto">
          <form onSubmit={handleSubmit}>
            ...
            <Group justify="flex-end" mt="md">
              <Button
                className="disabled:bg-slate-500 bg-red-700"
                type="submit"
                disabled={formData.name !== name}
              >
                Delete
              </Button>
            </Group>
